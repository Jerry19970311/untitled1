...
(function (R, f) {
    ...
    function L(_, B, $) {
        ...
        if ($ !== f) if (A) {
            ...
            if (1 === $ && C) return A[0]; else if (0 === $ && !C) return [A]
        } else if (0 === $) return [];
        return A
    }
    ...
    var s = {
        ...
        exec: function ($, I) {
            ...
            $ = $.trim();
            if ("" === $) return;
            if (D.ISSIMPLE.test($)) if (0 === $.indexOf("#") && typeof I.getElementById !== "undefined") return D.getElemById(I, $.substr(1)); else if (typeof I.getElementsByTagName !== "undefined") return T.util.toArray(I.getElementsByTagName($));
        ...
        operators: {
            "#": function (_, $) {
                ...
                var B = A.length;
                if (1 === B) return A[0].getElementsByTagName(_); else {
                    var $ = [], C = -1;
    ...
    T.dom = {
        wrapByArray: function ($) {
            if ($) if ($.nodeType !== f || $.setInterval) return [$]; else if ($.length) return T.util.toArray($);
            return []
            ...
            Q();
            if (T.dom.isReady) $.call(c, T); else V.push($);
            return this
        }
    };
    T.ready = T.dom.ready;
    var k = /\s*([:;])\s*/g, a = /[^:;]+?(?=:)/g, J = /[^:;]+/g, d = /[^\s]+/g, E = /-([a-z])/gi,
        Z = O.style.styleFloat !== f ? "styleFloat" : "cssFloat", o = /^float$/i;
    ...
    function t(A, _) {
        if ("" === this.style.cssText && "string" === typeof _) this.style.cssText = _; else for (var $ in A) this.style[$] !== f && (this.style[$] = A[$])
    }
    ...
    T.style = {
        fixStyleName: function ($) {
            ...
            var $ = T.style.parse(A);
            if (T.util.isArray($)) T.dom.eachNode(_, u, [$, $.length, A]); else T.dom.eachNode(_, t, [$, A]);
            return _
        }, removeCss: function (_, A) {
            var $ = T.style.parse(A);
            if (T.util.isArray($)) T.dom.eachNode(_, h, [$, $.length, A]); else T.dom.eachNode(_, m, [$]);
            return _
    ...
    function n(A, B, _) {
        ...
        if ($.attachEvent) $.attachEvent("on" + A, B); else if ($.addEventListener) $.addEventListener(A, B, false)
    }

    function g(_, A) {
        var $ = this;
        A = T.event.getDelegate($, _, A);
        if ($.detachEvent) $.detachEvent("on" + _, A); else if ($.removeEventListener) $.removeEventListener(_, A, false)
    }

    var H = /\s*,\s*/, D = 0;
    T.event = {
        idName: _ + "EventId", eventSpace: _ + "Events", addEvent: function (_, A, C, $) {
            A = A.split(H);
            var B = A.length;
    ...
    T.ajax = {
        createXhr: function () {
            ...
            if (H.headers) for (var F in H.headers) if ("content-type" === F.toLowerCase()) D.push(H.headers[F]); else A.setRequestHeader(F, H.headers[F]);
    ...
    T.cookie = {
        encoder: R.encodeURIComponent, decoder: R.decodeURIComponent, get: function (B, D) {
            var _ = T.cookie;
            B = _.encoder(B) + "=";
            ...
            if (A) {
                var H, $;
                if ("[object Date]" === S.call(A)) H = A; else {
